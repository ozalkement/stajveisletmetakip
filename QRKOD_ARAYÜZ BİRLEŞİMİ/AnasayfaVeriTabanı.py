...
class KareKod():
    def __init__(self):
        ...
        for index, row in isletme_listesi.iterrows():
            ...
            qr.add_data("{} ".format( isletmeAdi))
            qr.make(fit=True)
            img = qr.make_image(image_factory= qrcode.image.pil.PilImage, fill_color='black', back_color='white')
            img = img.resize((64,64), Image.ANTIALIAS)
            img.save('qrcode{}.png'.format(isletmeAdi))
            kanvas.drawImage('qrcode{}.png'.format(isletmeAdi),1.3*inch,1.3*inch)
            kanvas.showPage()
...
class TabRapor(QWidget):
    def __init__(self):
        ...
        dosyayaYaz=KareKod()
        kareKodButonu.clicked.connect(dosyayaYaz)
